...
export default function bosque() {
  ...
  function dispatch(action, payload, storeName) {
    if (storeName && !state.has(storeName)) {
      ...
      const actionName = /Symbol\((.+)\)/.exec(String(action))[1];
      /**
       * @todo: Only log this in development
       */
  ...
  class Store {
    ...
    set(updater, value) {
      ...
      console.log(`Store '${this._name}' updating ${isFunctionUpdater ? 'with function' : `'${updater}'`}`);
      if (isFunctionUpdater) {
    ...
    reset() {
      this.set((store) => store.merge(this.initialData));
    }
    ...
    withSavedPromise = (asyncFunc, name = 'pendingPromise') => {
      return (...args) => {
